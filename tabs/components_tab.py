...
def create_components_tab(notebook):
    ...
    tree.column("Код", width=100)
    tree.column("Цена", width=100)
    tree.column("Закупное", width=80)
    tree.column("Номер ТП", width=80)
    ...
    purchased_check = tk.Checkbutton(form_frame, variable=purchased_var)
    purchased_check.grid(row=1, column=1, padx=5, sticky="w")
    ...
    tk.Button(search_frame, text="Найти", command=lambda: load_components(search_entry.get())).pack(side=tk.LEFT, padx=5)
    tk.Button(search_frame, text="Сброс", command=lambda: (search_entry.delete(0, tk.END), load_components())).pack(side=tk.LEFT)

    # --- Определение всех функций перед кнопками ---
    def load_components(search_text=""):
        ...
        try:
            if search_text:
                cursor.execute("""
                    SELECT component_id, component_name, component_code, price, is_purchased, tech_process_number
                    FROM component
    ...
    def add_component():
        ...
        tp_num = tp_entry.get().strip() or None

        if not name:
            messagebox.showerror("Ошибка", "Наименование обязательно")
            return
        if price and not is_purchased:
            try:
        ...
        if tp_num:
            try:
                ...
                messagebox.showerror("Ошибка", "Номер техпроцесса должен быть целым числом")
                return
        ...
        try:
            cursor.execute("""
                INSERT INTO component (component_name, component_code, price, is_purchased, tech_process_number)
                VALUES (%s, %s, %s, %s, %s)
            """, (name, code, price_val, is_purchased, tp_num))
            conn.commit()
    ...
    def update_component():
        ...
        try:
            cursor.execute("""
                UPDATE component
                SET component_name=%s, component_code=%s, price=%s, is_purchased=%s, tech_process_number=%s
                WHERE component_id=%s
            """, (name, code, price_val, is_purchased, tp_num, comp_id))
            conn.commit()
            messagebox.showinfo("Успех", "Комплектующее обновлено")
            load_components()
            clear_form()
        except Exception as e:
            conn.rollback()
    ...
    def delete_component():
        ...
        if not selected:
            ...
            return
        comp_id = tree.item(selected[0])['values'][0]
        if messagebox.askyesno("Подтверждение", "Удалить комплектующее? Это может нарушить ссылочную целостность."):
            conn = connect_db()
            cursor = conn.cursor()
            try:
                cursor.execute("DELETE FROM component WHERE component_id=%s", (comp_id,))
                conn.commit()
    ...
    def on_select(event):
        ...
        if selected:
            ...
            code_entry.delete(0, tk.END); code_entry.insert(0, values[2] if values[2] else "")
            price_entry.delete(0, tk.END); price_entry.insert(0, values[3] if values[3] and values[3] != "None" else "")
            purchased_var.set(values[4] == "Да")
            tp_entry.delete(0, tk.END); tp_entry.insert(0, values[5] if values[5] and values[5] != "None" else "")

    # Привязка выбора
    tree.bind("<<TreeviewSelect>>", on_select)

    # Кнопки действий (после определения функций)
    btn_frame = tk.Frame(form_frame)
    btn_frame.grid(row=2, column=0, columnspan=6, pady=10)
    tk.Button(btn_frame, text="Добавить", command=add_component).pack(side=tk.LEFT, padx=5)
    tk.Button(btn_frame, text="Редактировать", command=update_component).pack(side=tk.LEFT, padx=5)
    tk.Button(btn_frame, text="Удалить", command=delete_component).pack(side=tk.LEFT, padx=5)
